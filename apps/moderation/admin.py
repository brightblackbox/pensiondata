...
class ModerationAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        created = False
        moderation.pre_save_handler(sender=self.model, instance=obj)
        if obj.pk is None:
            created = True
        obj.save()
        moderation.post_save_handler(sender=self.model, instance=obj, created=created)

        automoderate(obj, request.user)

    def delete_model(self, request, obj):
        # pre_delete.send(sender=self.model, instance=obj)
        # post_delete.send(sender=self.model, instance=obj)
        moderation.pre_delete_handler(sender=self.model, instance=obj)
        moderation.post_delete_handler(sender=self.model, instance=obj)
        automoderate(obj, request.user)
